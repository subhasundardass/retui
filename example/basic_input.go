	...
	return retui.Box(
		...
		components.Panel(
			...
			retui.Box(
				...
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
					components.NumberInput().ID("age").Placeholder("Enter age").
						Focused(isFocused(1)).Value(float64(age)).Min(1).Max(100).
						ArrowStep(false).
						OnChange(func(id string, value float64) { setAge(int(value)) }).
						Render(),
				),
			),
			...
			retui.Box(
				...
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
					components.NumberInput().ID("price").Placeholder("Price").
						Focused(isFocused(2)).Value(float64(price)).Decimals(2).
						ArrowStep(false).
						OnChange(func(id string, value float64) { setPrice(value) }).
						Render(),
				),
			),
			...
			retui.Box(
				retui.Props{Gap: 1},
				retui.NewStyle(),
				retui.Box(retui.Props{Width: retui.Fixed(10)}, retui.NewStyle(), retui.Text("From Date", retui.NewStyle())),
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
					components.DateInput().ID("fromDate").
						Focused(isFocused(3)).
						Value(fromDate).
						Format("DD/MM/YYYY").
			...
			retui.Box(
				retui.Props{Gap: 1},
				retui.NewStyle(),
				retui.Box(retui.Props{Width: retui.Fixed(10)}, retui.NewStyle(), retui.Text("To Date", retui.NewStyle())),
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
					components.DateInput().ID("toDate").
						Focused(isFocused(4)).
						Value(toDate).
						Format("DD/MM/YYYY").
						OnChange(func(id, value string) {
			...
			retui.Box(
				retui.Props{Gap: 1},
				retui.NewStyle(),
				retui.Box(retui.Props{Width: retui.Fixed(10)}, retui.NewStyle(), retui.Text("Password", retui.NewStyle())),
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
					components.Password().ID("password").
						Focused(isFocused(5)).
						Value(password).
						OnChange(func(id, value string) {
							setPassword(value)
			...
			retui.Box(
				retui.Props{Gap: 1},
				retui.NewStyle(),
				retui.Box(retui.Props{Width: retui.Fixed(10)}, retui.NewStyle(), retui.Text("Select", retui.NewStyle())),
				retui.Box(retui.Props{Width: retui.Fixed(30)}, retui.NewStyle(),
			...
			retui.Box(retui.Props{Padding: [4]int{0, 0, 0, 0}}, retui.NewStyle()),
			components.Button().
				ID("submit").
				Focused(isFocused(8)).
				Label("Submit").
